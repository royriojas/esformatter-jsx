...
module.exports = {
  ...
  stringBefore: function ( code ) {
    ...
    var ast = falafel( code, function ( node ) {
      ...
      if ( node.type === 'ExportNamedDeclaration' && node.source().indexOf( 'from' ) > -1 ) {
        node.update( node.source().replace( /^export/, 'import/*___fake_import_token__*/' ) );
      }
      if ( node.type === 'Decorator' ) {
        node.update( node.source().replace( /^\s*@/, '____decorator__at_sign___' ) );
      }
  ...
  stringAfter: function ( code ) {
    ...
    code = code.replace( /\/\*__async_token__\*\//g, 'async' );
