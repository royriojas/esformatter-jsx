...
module.exports = {
  ...
  stringBefore: function ( code ) {
    ...
    code = falafel( code, function ( node ) {
      if ( node.type === 'ClassProperty' ) {
        if ( node.value ) {
          var formattedProp = (node.static ? 'static ' : '') + (node.key.source() + ' = ' + node.value.source());
          node.update( formattedProp );
        }
      }

      if ( node.type === 'TemplateLiteral' && !findParent( node, 'JSXAttribute' ) ) {

        var idx = templateLiterals.length;
        var replaceString = '\'__TEMPORARY_STRING--PLACEHOLDER___NODE_(' + idx + ')\'';
  ...
  _restoreTemplateLiterals: function ( source ) {
    var me = this;
