...
function replaceJSXExpressionContainer( ast ) {
  var response = [ ];
  var index;
...
function iterateReverse( arr, cb ) {
  arr = arr || [ ];
...
function restoreContainers( source, containers ) {
  ...
  iterateReverse( containers, function ( entry, idx ) {
    ...
    source = source.replace( rx, container.replace( /^\{\s*/, '{ ' ).replace( /\s*\}$/, ' }' ) );
  } );
  return source;
}
  ...
  create: function ( htmlOptions, jsxOptions ) {
    ...
    var ins = {
      ...
      format: function ( ast ) {
        ...
        var containers = replaceJSXExpressionContainer( ast );
        ...
        source = restoreContainers( source, containers );
