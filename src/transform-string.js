...
const transformString = (str, opts) => {
  ...
  const avoidEscape = opts.avoidEscape;
  const quoteValue = opts.type === 'single' ? SINGLE_QUOTE : DOUBLE_QUOTE;
  const alternateQuote = opts.type !== 'single' ? SINGLE_QUOTE : DOUBLE_QUOTE;

  let content = str.substr(1, str.length - 2);
  let quote = quoteValue;
  let alternate = alternateQuote;

  const shouldAvoidEscape = avoidEscape &&
  ...
  const alternateEscape = new RegExp(`(^|[^\\\\])((?:\\\\{2})*)((?:\\\\${alternate})+)`, 'g');
  content = content.replace(alternateEscape, (input, boundaryChar, leadingEscapedSlashes, escapedQuoteChars) =>
  ...
  content = content.replace(quoteEscape, (input, boundaryChar, leadingEscapedSlashes, quoteChars) => boundaryChar + leadingEscapedSlashes + repeat(`\\${quote}`, quoteChars.length));
